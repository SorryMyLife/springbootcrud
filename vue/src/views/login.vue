...
import { httppost, httpget } from '../api/userMG'
    ...
    createCode(codeLength)
{
    ...
    getcode(){
      this.ruleForm.randomStr = this.createCode(26);
      httpget("/aa/crm/system-user/getcode/?cid="+this.ruleForm.randomStr).then((res)=>{
        console.log("res :: " ,res);
    ...
    submitForm(formName) {
      this.$refs[formName].validate(valid => {
        if (valid) {
          ...
          // }, 1000)
          var parm = JSON.stringify(this.ruleForm)
          httppost("/aa/crm/system-user/userlogin",parm).then((res)=>{
            if(res.code == 0){
              this.logining = false 
              document.cookie=''
            // document.cookie = "uid="+this.ruleForm.username+';expires=' + new Date().getDate() ;
              this.$store.commit('login', 'true')
